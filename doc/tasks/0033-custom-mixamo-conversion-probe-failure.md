...
- [ ] A reproduction exists: a custom Mixamo-rigged character (auto-rigged
      ...
      equivalent.
- [ ] Root cause identified through the disciplined loop (`ad-diagnose`):
...
- [ ] Conversion of the reproduction case succeeds: probes pass and the
      ...
      matrix.
- [ ] The failure message, when conversion still legitimately fails, tells a
...
- [ ] Reproduce: build/obtain a custom Mixamo auto-rigged character (no
      licensed assets committed), convert, capture the probe failure and the
      lifted-arm rest pose.
- [ ] Diagnose (`ad-diagnose`): rank hypotheses (twist/extra bones in
...
- [ ] Fix with regression tests at the level `tests/addon/` exercises;
